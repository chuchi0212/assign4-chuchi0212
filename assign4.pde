...
void setup() {
  ...
  for(int i = 0; i < 6; i++){
    float col = random(720);
    int row = floor(random(4));
    soldierX[i] = col;
    soldierY[i] = row* SOIL_SIZE+i*320;
  }
  ...
  for(int i = 0; i < 6; i++){
    ...
    cabbageY[i] = row* SOIL_SIZE+i*320;
  }
}
...
void draw() {
  ...
  switch (gameState) {
        ...
        if(leftState){
          ...
          if(playerRow < SOIL_ROW_COUNT - 1){
            ...
            if(soilHealth[playerCol][playerRow+1] > 0){
              soilHealth[playerCol][playerRow+1] -= digSpeed;
            }  
          }
        }
      }
    ...
    for(int i = 0; i < 6; i++){
      ...
      soldierX[i] %= 720;
      if(playerX < soldierX[i] && playerX + 80 > soldierX[i] - 80 &&
